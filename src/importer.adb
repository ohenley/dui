                ...
                for i in 1 .. 3 loop
                    ...
                    begin
                        ...
                        null;
                            case i is
                                when 1 =>
                                    img (col, row).r := g.color_val(float'value (to_string (color_value))/ 255.0);
                                when 2 =>
                                    img (col, row).g := g.color_val(float'value (to_string (color_value))/ 255.0);
                                when 3 =>
                                    img (col, row).b := g.color_val(float'value (to_string (color_value))/ 255.0);
                            end case;
                        end if;
                    end;
                end loop;
